...
export function Users(){
    // https://task-master-api-five.vercel.app/admin/deleteuser
    const navigate = useNavigate();
    const [users,setUsers] = useRecoilState(UsersList);
    const removeUser = async(userId)=>{
        const response = await fetch(`https://task-master-api-five.vercel.app/admin/deleteuser?userId=${userId}`,{
            method : 'DELETE',
            headers : {
        ...
        if(data.success){
            const response = await fetch('https://task-master-api-five.vercel.app/admin/getusers',{
                method : 'GET',
                headers : {
                    "Content-Type" : "application/json",
                    authorization : "Bearer " + JSON.parse(localStorage.getItem('Admintoken'))
