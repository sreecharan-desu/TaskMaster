...
export default function AdminProfile(){
    ...
    const UpdateDetails = ()=>{
    // https://task-master-api-five.vercel.app/admin/update (PUT) 
    const bodyData = JSON.stringify({username : usernameProfile, password : password });
        const callDB=async()=>{
            try{
                const response = await fetch('https://task-master-api-five.vercel.app/admin/update',{
                    method: 'PUT',
                    headers: {
    ...
    useEffect(()=>{
        const fecthUsername = async()=>{
            const response = await fetch('https://task-master-api-five.vercel.app/admin/details',{
                method : 'GET',
                headers : {
                    'Content-Type' : 'application/json',
                    authorization : "Bearer " + JSON.parse(localStorage.getItem('Admintoken'))
